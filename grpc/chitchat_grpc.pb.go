...
const (
	ChitChat_Chat_FullMethodName = "/chitchat.ChitChat/Chat"
)
...
//
// The bidi streaming RPC. Each client opens one stream.
type ChitChatClient interface {
	// A long-lived bidirectional stream:
	// - Client sends JOIN -> server replies with a Welcome (as a ServerEvent of type SYSTEM with content set).
	// - Client sends CHAT/LEAVE -> server broadcasts to all.
	// - Server may push SYSTEM messages at any time (joins/leaves/announcements).
...
func (c *chitChatClient) Chat(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ClientEvent, ServerEvent], error) {
...
type ChitChat_ChatClient = grpc.BidiStreamingClient[ClientEvent, ServerEvent]
...
func (UnimplementedChitChatServer) Chat(grpc.BidiStreamingServer[ClientEvent, ServerEvent]) error {
...
func _ChitChat_Chat_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ChitChatServer).Chat(&grpc.GenericServerStream[ClientEvent, ServerEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChitChat_ChatServer = grpc.BidiStreamingServer[ClientEvent, ServerEvent]
